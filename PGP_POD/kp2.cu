...
class InputMPI final {
public:
    InputMPI(
        std::istream& is,
        const int rank
    ) : stream(is), is_root(rank == 0) {}

    friend InputMPI &operator>>(InputMPI &input, flt &value) {
        if (input.is_root)
            input.stream >> value;
    ...
    friend InputMPI &operator>>(InputMPI &input, int &value) {
        if (input.is_root)
            input.stream >> value;
    ...
    friend InputMPI &operator>>(InputMPI &input, string &str){
        if (input.is_root)
            input.stream >> str;
        int size = str.size();
        MPI_Bcast(&size, 1, MPI_INT, 0, MPI_COMM_WORLD);
        str.resize(size);
        MPI_Bcast(const_cast<char *>(str.data()), size,
...
int main(int argc, char **argv) {
    if (argc >= 3) {
        cout << "argc must be less or equal than 2\n";
        exit(EXIT_FAILURE);
    }
    bool flag_cuda;
    if (argc == 1)
    ...
    InputMPI mpi_input(cin, rank);

    flt three_points_1, three_points_2,
        phi, r_first_0, z_first_0, f_first_0, A_first_r, A_first_z,
        w_first_r, w_first_z, w_first_f, p_first_r, p_first_z,
        r_second_0, z_second_0, f_second_0, A_second_r, A_second_z,
        w_second_r, w_second_z, w_second_f, p_second_r, p_second_z,
        rds, r_, trs, lght, tro;
    ...
    string directory;
    mpi_input >> iter_count >> directory >> width >> height >> phi
        >> r_first_0 >> z_first_0 >> f_first_0
        >> A_first_r >> A_first_z
        >> w_first_r >> w_first_z >> w_first_f >> p_first_r >> p_first_z
        >> r_second_0 >> z_second_0 >> f_second_0
        >> A_second_r >> A_second_z
        >> w_second_r >> w_second_z >> w_second_f >> p_second_r >> p_second_z;

    uchar4 * const data = (uchar4 *) malloc(width * height * sizeof(uchar4));
    uchar4 *deviceData;
    cudaErrorCheck(cudaMalloc(&deviceData, width * height * sizeof(uchar4)));
    cudaErrorCheck(cudaMemset(deviceData, 0, width * height * sizeof(uchar4)));

    vector<triangle> triangles;

    mpi_input >> cntr.x >> cntr.y >> cntr.z
        >> clr.x >> clr.y >> clr.z
        >> rds >> r_ >> trs >> lght;
    ...
    mpi_input >> tp_1.x >> tp_1.y >> tp_1.z
        >> tp_2.x >> tp_2.y >> tp_2.z
        >> tp_3.x >> tp_3.y >> tp_3.z
        >> tp_4.x >> tp_4.y >> tp_4.z
        >> trash
        >> tc.x >> tc.y >> tc.z >> tro;
    ...
    mpi_input >> sunshine.x >> sunshine.y >> sunshine.z
        >> sunshine_c.x >> sunshine_c.y >> sunshine_c.z
        >> three_points_1 >> three_points_2;
    sunshine.x = sunshine.y = sunshine.z = 100;

    triangle *deviceTriangles;
    cudaMalloc(&deviceTriangles, triangles.size() * sizeof(triangle));
    cudaMemcpy(deviceTriangles, triangles.data(),
        triangles.size() * sizeof(triangle), cudaMemcpyHostToDevice
    );

    for (int iter = 0; iter < iter_count; ++iter) {
        const flt step = 2.0F * FLT_PI * iter / iter_count,
            r_first = A_first_r * sinf(w_first_r * step + p_first_r) + r_first_0,
            z_first = A_first_z * sinf(w_first_z * step + p_first_z) + z_first_0,
            f_first = w_first_f * step + f_first_0,
            r_second = A_second_r * sinf(w_second_r * step + p_second_r) + r_second_0,
            z_second = A_second_z * sinf(w_second_z * step + p_second_z) + z_second_0,
            f_second = w_second_f * step + f_second_0;

        pc.x = cosf(f_first) * r_first;
        pc.y = sinf(f_first) * r_first;
        pc.z = z_first;

        pv.x = cosf(f_second) * r_second;
        pv.y = sinf(f_second) * r_second;
        pv.z = z_second;

        if (flag_cuda) {
            render<<<dim3(16, 16), dim3(16, 16)>>>(
                pc, pv, width, height, phi, deviceData, sunshine, sunshine_c,
                deviceTriangles, triangles.size()
            );
            cudaErrorCheck(cudaGetLastError());
            cudaErrorCheck(cudaMemcpy(data, deviceData,
                sizeof(uchar4) * width * height, cudaMemcpyDeviceToHost
            ));
        } else
            render_cpu(
                pc, pv, width, height, phi, data, sunshine, sunshine_c,
                triangles.data(), triangles.size()
            );

        const string filename(directory + '/' + to_string(iter) + ".data");
        cout << iter << ": " << filename << endl;
        ofstream stream(
            filename,
            ios_base::binary | ios_base::out | ios_base::trunc
        );
        stream.write(reinterpret_cast<char *>(&width), sizeof(int) * 1U);
        stream.write(reinterpret_cast<char *>(&height), sizeof(int) * 1U);
        stream.write(reinterpret_cast<char *>(data), sizeof(uchar4) * width * height);
    }
